    ...
    public function student(Request $request)
    {
        if ($request->isMethod('post')){
            ...
            elseif ($request->_type == 'data' && $request->_data == 'student'){
                ...
                $student->student_birthday = $student->birthday();
                $student->student_gender = ['id' => $student->student_gender, 'text' => Gender::find($student->student_gender)->value('gender_name')];
                $student->student_religion = ['id' => $student->student_regligion, 'text' => Religion::find($student->student_religion)->value('religion_name')];
                $student->student_civic = ['id' => $student->student_civic, 'text' => Civic::find($student->student_civic)->value('civic_name')];
                $student->student_hobby = ['id' => $student->student_hobby, 'text' => Hobby::find($student->student_hobby)->value('hobby_name')];
                $student->student_purpose = ['id' => $student->student_purpose, 'text' => Purpose::find($student->student_purpose)->value('purpose_name')];
                $student->student_residence = ['id' => $student->student_residence, 'text' => Residence::find($student->student_residence)->value('residence_name')];
                $student->student_province = ['id' => $student->student_province, 'text' => Territory::province($student->student_province)];
                $student->student_distric = ['id' => $student->student_distric, 'text' => Territory::distric($student->student_distric)];
                $student->student_subdistric = ['id' => $student->student_subdistric, 'text' => Territory::subdistric($student->student_subdistric)];
                $student->student_village = ['id' => $student->student_village, 'text' => Territory::village($student->student_subdistric, $student->student_village)];
                $student->student_distance = ['id' => $student->student_distance, 'text' => Distance::find($student->student_distance)->value('distance_name')];
                $student->student_transport = ['id' => $student->student_transport, 'text' => Transport::find($student->student_transport)->value('transport_name')];
                $student->student_travel = ['id' => $student->student_travel, 'text' => Travel::find($student->student_travel)->value('travel_name')];
                $student->student_program = ['id' => $student->student_program, 'text' => Program::find($student->student_program)->value('program_name')];
                $student->student_father_status = ['id' => $student->student_father_status, 'text' => ParentStatus::name($student->student_father_status)];
                $student->student_mother_status = ['id' => $student->student_mother_status, 'text' => ParentStatus::name($student->student_mother_status)];
                $student->student_father_birthday = $student->fatherbirthday();
                $student->student_mother_birthday = $student->motherbirthday();
                $student->student_father_study = ['id' => $student->student_father_study, 'text' => Study::name($student->student_father_study)];
                $student->student_mother_study = ['id' => $student->student_mother_study, 'text' => Study::name($student->student_mother_study)];
                $student->student_father_job = ['id' => $student->student_father_job, 'text' => Job::name($student->student_father_job)];
                $student->student_mother_job = ['id' => $student->student_mother_job, 'text' => Job::name($student->student_mother_job)];
                $student->student_father_earning = ['id' => $student->student_father_earning, 'text' => Earning::name($student->student_father_earning)];
                $student->student_mother_earning = ['id' => $student->student_mother_earning, 'text' => Earning::name($student->student_mother_earning)];
                $student->student_guard_birthday = $student->guardbirthday();
                $student->student_guard_study = ['id' => $student->student_guard_study, 'text' => Study::name($student->student_guard_study)];
                $student->student_guard_job = ['id' => $student->student_guard_job, 'text' => Study::name($student->student_guard_job)];
                $student->student_guard_earning = ['id' => $student->student_guard_earning, 'text' => Study::name($student->student_guard_earning)];
