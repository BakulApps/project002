<div class="navbar navbar-expand-md navbar-dark">
    <div class="navbar-brand">
        <a href="{{auth('student')->check() ? route('student.backend.home') : route('student.home')}}" class="d-inline-block">
        ...
        <ul class="navbar-nav navbar-right">
            @if(session()->has('student.auth'))
                <li class="nav-item dropdown dropdown-user">
                    <a href="#" class="navbar-nav-link d-flex align-items-center dropdown-toggle" data-toggle="dropdown">
                        @if(session()->get('student.auth')->student_photo_file == '')
                        ...
                        @endif
                        <span>{{session()->get('student.auth')->student_name}}</span>
                    </a>
            ...
            @if(auth('student')->check())
                <li class="nav-item dropdown dropdown-user">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <a href="{{route('student.backend.setting')}}" class="dropdown-item"><i class="icon-cog"></i> Pengaturan</a>
                        <a href="{{route('student.backend.logout')}}" class="dropdown-item"><i class="icon-switch2"></i> Keluar</a>
                    </div>
                </li>
            @endif
        </ul>
